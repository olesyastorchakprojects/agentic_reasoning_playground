...
- eval_run_id: `ffec4e03-3b0f-4862-8943-4010cb1d8f12`
- run_type: `golden_dataset`
- status: `completed`
- started_at: `2026-05-08 21:29:34.959532634 UTC`
- completed_at: `2026-05-08 21:30:46.323740536 UTC`
- runtime_run_count: `1`
...
|---|---:|---|
| usable_first_response_rate | 0.5000 | Share of runs where the final answer can be shown as a first diagnostic response |
...
| final_answer_judge_score | 1.4000 | Judge-based quality of final diagnostic response |

## Judge-Based Aggregated Metrics

| metric | value | formula |
|---|---:|---|
| usable_first_response_rate | 0.5000 | frac(FA1≥1 ∧ FA2≥1 ∧ FA5≥1) |
| query_structuring_judge_score | 0.7500 | mean over runs of avg(QS1, QS2) |
| evidence_pack_judge_score | 0.7500 | mean over runs of avg(EP1, EP2) |
| final_answer_judge_score | 1.4000 | mean over runs of avg(FA1, FA2, FA3, FA4, FA5) |
| query_structuring_no_hard_fail_rate | 1.0000 | frac(QS1>0 ∧ QS2>0) |
| evidence_pack_no_hard_fail_rate | 1.0000 | frac(EP1>0 ∧ EP2>0) |
| final_answer_no_hard_fail_rate | 0.5000 | frac(FA1>0 ∧ FA2>0 ∧ FA4>0 ∧ FA5>0) |
| diagnostic_move_hard_fail_rate | 0.5000 | 1 − final_answer_no_hard_fail_rate |
...
|---|---:|---:|---:|
| final_no_root_cause_claim | 1 | 0 | 1 |
| final_first_check_discriminates | 1 | 0 | 1 |
...
|---|---:|---:|
| final_no_root_cause_claim | 1 | 0.5000 |
| final_first_check_discriminates | 1 | 0.5000 |
...
| bad_final_with_good_query_and_evidence_rate | 0.5000 | frac(!usable ∧ QS1>0 ∧ QS2>0 ∧ EP1>0 ∧ EP2>0) |
...
| final answer | usable 50%, judge 1.40, no-hard-fail 50% | mixed | 1 premature certainty (FA1=0); 1 vague first check (FA2=0); 1 partial source alignment (FA3<2) |

### Failure Path

1 of 2 responses were unusable.

- 1 unusable despite good query + evidence → **final answer stage failure**
  - 1 × FA1=0: premature certainty or root cause claim
  - 1 × FA2=0: vague or non-discriminating first check

Main observed weakness: **retrieval** (composite 0.00).
...
|---|---|---:|---:|
| `d541773c-a7e2-42e0-bd59-44ddbfb8b29f` | `b5bdb5ea-1e56-42d3-829a-8482e562825d` | 1.2000 | false |
...
|---|---:|---:|---:|---:|
| continuation_hypothesis_update_discipline | 1689 | 481 | 2170 | 0.000181 |
| continuation_next_check_progression | 1172 | 358 | 1530 | 0.000130 |
| continuation_observation_resolution_context_recovery | 1059 | 391 | 1450 | 0.000131 |
| continuation_problem_understanding_update | 1389 | 346 | 1735 | 0.000139 |
| evidence_pack_role_fit | 2884 | 706 | 3590 | 0.000285 |
| evidence_pack_sufficiency | 4104 | 738 | 4842 | 0.000353 |
| final_alternative_context_handling | 9736 | 583 | 10319 | 0.000603 |
| final_first_check_discriminates | 9858 | 1325 | 11183 | 0.000758 |
| final_hypothesis_source_alignment | 8338 | 1776 | 10114 | 0.000772 |
| final_no_root_cause_claim | 9766 | 697 | 10463 | 0.000628 |
| final_result_interpretation_usefulness | 1775 | 1289 | 3064 | 0.000347 |
| query_structuring_field_boundary_correctness | 713 | 419 | 1132 | 0.000119 |
| query_structuring_grounding_conservatism | 683 | 1190 | 1873 | 0.000272 |
...
| judge_total | 53166 | 10299 | 63465 | 0.004718 |
| run_total | 61682 | 14830 | 76512 | 0.005514 |

Run total cost usd = runtime total cost usd + judge total cost usd = 0.000796 + 0.004718 = 0.005514
