...
pub fn validate_request(req: &ModelGenerationRequest) -> Result<(), ModelClientError> {
    if req.messages.is_empty() {
        return Err(ModelClientError::InvalidRequest("messages must not be empty"));
    }
    for msg in &req.messages {
        if msg.content.trim().is_empty() {
            return Err(ModelClientError::InvalidRequest("message content must not be empty"));
        }
    }
    ...
    if let Some(max_tokens) = req.max_output_tokens {
        if max_tokens == 0 {
            return Err(ModelClientError::InvalidRequest("max_output_tokens must be > 0"));
        }
    }
    Ok(())
}
